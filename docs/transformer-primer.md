...
**ヘッド（Head）** とは、Self-Attentionを実行する独立した計算ユニットのことです。
1つのヘッドは専用の `Q/K/V` 重み行列を持ち、入力系列に対して1回のAttention計算を行います。
ただし1つのヘッドだけでは、1種類の注目パターンしか学習できません。

Multi-Head Attentionでは、埋め込みベクトルをヘッド数で分割し、各ヘッドが独立にAttentionを実行します。
ヘッドごとに異なる観点の関係（語尾、子音パターン、位置依存など）を捉えやすくなります。
全ヘッドの出力を結合した後、射影行列 `Wo` で元の次元に戻します。

`microgpt` では:

- `N_EMBD = 16`（埋め込み次元）
- `N_HEAD = 4`（ヘッド数）
- `HEAD_DIM = 4`（1ヘッドあたりの次元 = `N_EMBD / N_HEAD`）

で、16次元を4ヘッドに分割しています。各ヘッド（H0〜H3）は4次元の部分空間で独立にAttentionを計算し、結果を結合して `attn_wo` で射影します。
